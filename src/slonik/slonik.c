...
load_slony_base(SlonikStmt * stmt, int no_id)
{
	...
	else if ((adminfo->pg_version >= 90000) && (adminfo->pg_version < 120000)) /* 12 */
	{
		/**
		 * 9.0 through 12 are so far just like 8.4
		 **/
		use_major = 8;
		use_minor = 4;
	}
...
load_slony_functions(SlonikStmt * stmt, int no_id)
{
	...
	else if ((adminfo->pg_version >= 90000) && (adminfo->pg_version < 120000)) /* 9.0, 9.1, 9.2 */
	{
		/**
